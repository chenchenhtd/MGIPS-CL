...
class MMPNERProcessor(object):
    ...
    def load_file_to_array(self, load_file, raw_words=None, raw_targets=None, imgs=None):
    ...
    def load_from_file(self, mode="train"):
        ...
        imgs = []
        split_index_dev = -653 #-870/-653
        split_index_test = -653 #-3000/-653
        self.load_file_to_array(load_file, raw_words, raw_targets, imgs)
        if mode=="train":        
            load_file = os.path.join(self.cwd,self.data_path["dev"])
            self.load_file_to_array(load_file, raw_words, raw_targets, imgs)
            raw_words = raw_words[0:split_index_dev]
            raw_targets=raw_targets[0:split_index_dev]
            imgs=imgs[0:split_index_dev]
        if mode=="train":        
            load_file = os.path.join(self.cwd,self.data_path["test"])
            self.load_file_to_array(load_file, raw_words, raw_targets, imgs)
            raw_words = raw_words[0:split_index_test]
            raw_targets=raw_targets[0:split_index_test]
            imgs=imgs[0:split_index_test]
        ...
        aux_imgs = torch.load(aux_path, weights_only=True)

        rcnn_imgs = torch.load(os.path.join(self.cwd,self.data_path["img2crop"]), weights_only=True)
