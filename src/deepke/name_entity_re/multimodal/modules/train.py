...
class Trainer(object):
        ...
        # with tqdm(total=3180, postfix='loss:{0:<6.5f}', leave=False, dynamic_ncols=True, initial=self.step) as pbar:
            ...
            for epoch in range(1, self.args.num_epochs+1):
                ...
                for batch in self.train_data:
                    ...
                    attention_mask, labels,labels_fx, logits, loss, sequence_output = self._step(batch, mode="train")
            ...
            self.logger.info("***********************************************************************************")
            self.logger.info("the start constrast lr is {},the start hiera lr is {},the weight_decay is {}".format(self.args.con_lr,self.args.hie_lr,self.args.weight_decay))
        ...
        with torch.no_grad():
            with tqdm(total=len(self.dev_data), leave=False, dynamic_ncols=True) as pbar:
                ...
                for batch in self.dev_data:
                    ...
                    attention_mask, labels,labels_fx , logits, loss, sequence_output = self._step(batch, mode="dev")    # logits: batch, seq, num_labels
    ...
    def predict(self):
                        ...
                        for j, m in enumerate(mask):
                            ...
                            if m:
                                if label_map[label_ids[i][j]] != "X" and label_map[label_ids[i][j]] != "[SEP]":
                                    ...
                                    if label_ids[i][j] == logits[i][j] and label_map[label_ids[i][j]] != "O":
                                    # if label_map[label_ids[i][j]] != "O":
                                        token_vercs.append(sequence_output[i,j])
                                        entity_labels.append(label_map[label_ids[i][j]])
                            else:
                                break
                        y_true.append(temp_1)
                        y_pred.append(temp_2)
        ...
        return token_vercs, entity_labels
        
    def _step(self, batch, mode="train"):
        input_ids, token_type_ids, attention_mask, labels, labels_fx,images,aux_imgs= batch
        rcnn_imgs=None
        logits, loss, sequence_output = self.model(input_ids=input_ids, attention_mask=attention_mask, token_type_ids=token_type_ids, labels=labels, images=images,aux_imgs=aux_imgs,rcnn_imgs=rcnn_imgs)
        return attention_mask, labels,labels_fx, logits, loss, sequence_output
    ...
    def multiModal_before_train(self):
        ...
        params = {'lr': self.args.hie_lr, 'weight_decay': self.args.weight_decay}
        params['params'] = []
        for name, param in self.model.named_parameters():
            if 'cross' in name:
                params['params'].append(param)
        ...
        params = {'lr': self.args.con_lr, 'weight_decay': self.args.weight_decay}
        params['params'] = []
        for name, param in self.model.named_parameters():
